...
async function handler(req,res){const started=performance.now();try{
 if(req.method==='POST'&&req.url==='/api/agent'){const auth=req.headers.authorization||'';const request=await body(req);if(!auth)return json(res,401,{error:'Agent authorization is required'},req);const result=await executeAgent(request,auth);return json(res,result.status,result.body,req);}
