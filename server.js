...
async function handler(req,res){try{
 ...
 if(req.method==='POST'&&req.url==='/api/large-js-chat'){const b=await body(req);return json(res,200,largeJavaScriptLM.answer(b.message||''));}
 ...
 if(req.method==='POST'&&req.url==='/api/image')return json(res,501,{error:'Remote image generation is disabled in pure-JavaScript mode'});
 if(req.method==='POST'&&req.url==='/api/search')return json(res,501,{error:'External search is disabled in pure-JavaScript mode'});
