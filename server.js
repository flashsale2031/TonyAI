...
async function cachedEvidenceSearch(query,maxResults=8){const key=String(query||'').trim().toLowerCase();const hit=evidenceCache.get(key);if(hit&&Date.now()-hit.created<EVIDENCE_CACHE_MS)return{...hit.value,cache:'hit'};const value=await evidenceSearch(query,{maxResults});evidenceCache.set(key,{created:Date.now(),value});return{...value,cache:'miss'};}
