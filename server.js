...
import { collectResults } from './engine/results-collection.js';
...
function compress(body,acceptEncoding){const source=Buffer.isBuffer(body)?body:Buffer.from(body);if(source.length<512)return{body:source,encoding:null};if(/br/i.test(acceptEncoding||''))return{body:brotliCompressSync(source,{params:{11:5}}),encoding:'br'};if(/gzip/i.test(acceptEncoding||''))return{body:gzipSync(source,{level:6}),encoding:'gzip'};return{body:source,encoding:null};}
...
async function cachedEvidenceSearch(query,maxResults=8){const key=String(query||'').trim().toLowerCase();const hit=evidenceCache.get(key);if(hit&&Date.now()-hit.created<EVIDENCE_CACHE_MS)return{...hit.value,cache:'hit'};const value=selectMostCredibleResponse(await evidenceSearch(query,{maxResults}));const pages=Array.isArray(value?.results)&&value.results.length?value.results:value?.verifiedSources||[];const collection=collectResults(query,pages,{limit:maxResults});const collected=collection.toJSON();const merged={...value,...collected,answer:value.answer,result:value.result,selectedSource:value.selectedSource};evidenceCache.set(key,{created:Date.now(),value:merged});return{...merged,cache:'miss'};}
function agentCookie(req){const cookies=String(req.headers.cookie||'').split(';').map(x=>x.trim());const item=cookies.find(x=>x.startsWith('tonyai_agent='));return item?decodeURIComponent(item.slice('tonyai_agent='.length)):'';}
async function handler(req,res){const started=performance.now();try{
 if(req.method==='POST'&&req.url==='/api/agent'){const request=await body(req);const auth=req.headers.authorization||agentCookie(req);const normalized=auth.startsWith('Bearer ')?auth:`Bearer ${auth}`;if(!auth)return json(res,401,{error:'Agent authorization is required'},req);const result=await executeAgent(request,normalized);return json(res,result.status,result.body,req);}
 ...
 if(req.method==='GET'&&req.url==='/api/diagnostics')return json(res,200,capabilityRuntime.diagnostics(),req,'private, max-age=5');
 if(req.method==='GET'&&req.url==='/api/memory')return json(res,200,{memories:capabilityRuntime.memory.list()},req,'private, max-age=5');
 if(req.method==='POST'&&req.url==='/api/memory'){const b=await body(req);if(b.action==='add')return json(res,200,capabilityRuntime.memory.add(b.text,{explicit:b.explicit===true,ttlMs:Number(b.ttlMs)||0}),req);if(b.action==='remove')return json(res,200,{ok:capabilityRuntime.memory.remove(b.id)},req);if(b.action==='clear'){capabilityRuntime.memory.clear();return json(res,200,{ok:true},req);}return json(res,400,{error:'action must be add, remove, or clear'},req);}
 ...
 if(req.method==='GET'&&req.url==='/api/capabilities')return json(res,200,{...assistant.capabilities(),functionalReplica:true,binaryReplacement:binaryReplacementStatus(),localBrain:true,localLanguageModel:true,largeJavaScriptLM:true,largeJavaScriptLMParameters:LARGE_JS_LM_PARAMETER_CAPACITY,largeJavaScriptLMParameterMode:'virtual-sparse-capacity',largeJavaScriptLMPrimaryBackbone:'pure-javascript',largeJavaScriptLMPretrainedRequired:false,largeJavaScriptLMExternalNeuralModel:false,largeJavaScriptLMExternalGenerationAPI:false,openAIRequired:false,provider:tonyAIProvider.capabilities(),pureJavaScriptMode:true,externalSearchEnabled:true,externalImageGenerationEnabled:true,chatPrimary:'large-js-primary',chatToolOrchestration:true,requestRouting:true,capabilityRecovery:true,diagnostics:true,userControlledMemory:true,cacheManager:true,chatResponseSearchEngine:'DuckDuckGo DOM via chatresponse.js',evidenceSearchEngine:'DuckDuckGo DOM + page inspection via evidence-search.js',chatResponseDomInspection:true,chatResponseFrequencyMap:true,resultsCollection:true,resultsCollectionDocument:true,performance:{keepAlive:true,immutableEngineAssets:true,cachedInferenceHotPaths:true,compressedResponses:true,pageCache:true,chatResponseBudgetMs:CHAT_TIMEOUT_MS,chatResponseCacheMs:CHATRESPONSE_CACHE_MS,evidenceSearchCacheMs:EVIDENCE_CACHE_MS},agentExecution:true,agentActions:['diagnostics','list','read','write','run','github-read','github-write'],githubConnector:true,githubOwner:process.env.GITHUB_OWNER||'flashsale2031',githubRepo:process.env.GITHUB_REPO||'TonyAI',githubConfigured:Boolean(process.env.GITHUB_TOKEN),agentAuthConfigured:Boolean(process.env.TONY_AGENT_TOKEN)},req,'public, max-age=30, stale-while-revalidate=300');
 ...
 if(req.method==='GET'&&(req.url==='/'||req.url==='/index.html')){const headers={'content-type':'text/html; charset=utf-8','cache-control':`public, max-age=${PAGE_MAX_AGE}, stale-while-revalidate=600`};if(process.env.TONY_AGENT_TOKEN)headers['set-cookie']=[`tonyai_agent=${encodeURIComponent(process.env.TONY_AGENT_TOKEN)}; Path=/; HttpOnly; Secure; SameSite=Strict`];return send(res,200,await page(),headers,req);}
