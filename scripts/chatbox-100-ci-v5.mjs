...
if (questions.length !== 100) throw new Error(`Expected exactly 100 CI questions, got ${questions.length}`);
...
try {
  ...
  // Keep CI independent of Duck.ai's external frontend while still verifying the
  // real cross-origin destination URL and exact query injection from TonyAI.
  ...
  const composer = page.getByLabel('Message TONY');
  ...
  for (let index = 0; index < 100; index += 1) {
    ...
    if (actual.origin !== 'https://duck.ai' || !['/', '/chat'].includes(actual.pathname) || injected !== question || (prompt !== null && prompt !== '1')) {
