...
async function waitForServer() {
  ...
  while (Date.now() < deadline) {
    try { if ((await fetch(`${BASE}/`, { signal: AbortSignal.timeout(1200) })).ok) return; } catch {}
    await sleep(200);
  }
  throw new Error('TonyAI server did not become ready');
}
...
function validateAnswer(question, answer) {
  ...
  const candidates = [...new Set(tokens(question).filter(w => w.length >= 4 && !ignored.has(w)))];
  if (candidates.length && !candidates.some(w => a.includes(w))) return 'answer has no meaningful overlap with question';
  return null;
}
...
try {
  ...
  for (let index = 0; index < 100; index += 1) {
    ...
    const injectedUrl = await page.evaluate(q => window.TonyAIChatResponse?.duckAiUrl?.(q), question);
    if (!injectedUrl) throw new Error(`Question ${index + 1}/100 did not expose Duck.ai URL generation`);
    const expected = new URL('https://duck.ai/chat');
    expected.searchParams.set('prompt', '1');
    expected.searchParams.set('q', question);
    if (injectedUrl !== expected.toString()) throw new Error(`Question ${index + 1}/100 generated incorrect Duck.ai URL: ${injectedUrl}`);
