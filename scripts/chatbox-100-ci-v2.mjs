...
if (!match) throw new Error('Could not extract canonical question suite');
const questions = Function(`return (${match[1]})`)();
if (!Array.isArray(questions) || questions.length !== 100) throw new Error(`Expected 100 questions, got ${questions.length}`);
...
const validateAnswer = (question, answer) => {
  ...
  const arithmetic = [
    [/\bwhat is 2\s*\+\s*2\b/, '\\b4\\b', '2+2'],
    [/\bwhat is 10\s*\*\s*10\b/, '\\b100\\b', '10*10'],
    [/\bwhat is 15\s*[-+]\s*7\b/, '\\b8\\b', '15-7'],
    [/\bwhat is 12\s*\/\s*3\b/, '\\b4\\b', '12/3']
  ];
  for (const [pattern, expected, label] of arithmetic) if (pattern.test(q) && !new RegExp(expected).test(a)) return `${label} answer missing`;
...
try {
  ...
  await context.route('https://duck.ai/**', route => route.fulfill({
    status: 200,
  ...
  for (let index = 0; index < questions.length; index += 1) {
    ...
    const q = actual.searchParams.get('q');
    const prompt = actual.searchParams.get('prompt');
    if (actual.origin !== 'https://duck.ai' || !['/','/chat'].includes(actual.pathname) || q !== question || (prompt !== null && prompt !== '1')) {
