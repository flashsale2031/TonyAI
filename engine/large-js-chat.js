...
export async function largeJSChat({messages=[],attachments=[]}={},assistant=null){
 ...
 if(wantsImage){try{const image=await imageTool(latest);toolResults.push({tool:'image-generation',ok:image.ok,error:image.error,summary:image.ok?'Image artifact generated by the optional external image tool.':image.error});if(image.ok){artifacts={images:[image]};reply=`Image generated successfully (${image.model}, ${image.size}).`;confidence=Math.max(confidence,.8);}}catch(e){toolResults.push({tool:'image-generation',error:String(e.message||e)});}}
 if(!wantsImage){
