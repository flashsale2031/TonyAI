...
export async function largeJSChat({messages=[],attachments=[]}={},assistant=null){
 ...
 const needsEvidence=!reply&&!wantsImage&&wants(latest,/\b(search|look up|latest|today|current|news|research|online|internet|sources?|what is|who is|where is|when is|how much|how many|compare|comparison|price|cost)\b/i);
 ...
 return{reply,confidence,requiresHuman:false,source:'large-js-primary',provider:tonyAIProvider.capabilities(),largeJavaScriptLM:true,primaryGenerationBackbone:'pure-javascript-sparse-neural-symbolic',pretrainedModel:false,externalNeuralModel:false,externalGenerationAPI:false,openAIRequired:false,parameterCapacity:450_000_000_000,toolResults,sources,artifacts,attachmentCount:Array.isArray(attachments)?attachments.length:0,stats:largeJavaScriptLM.stats()};
}
