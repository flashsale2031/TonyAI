...
export async function largeJSChat({messages=[],attachments=[]}={},assistant=null){
 ...
 // Deterministic answers have already been verified by a local tool. Do not spend
 // network time searching for the same calculation/text transformation.
 const needsEvidence=!reply&&wants(latest,/\b(search|look up|latest|today|current|news|research|online|internet|sources?|what is|who is|where is|when is|how much|how many|compare|comparison|price|cost)\b/i);
 if(needsEvidence){try{const web=await duckduckgoSearch(latest,{maxResults:12});sources=web.results||[];if(sources.length){const evidence=sourcesText(sources);largeJavaScriptLM.addKnowledge([{id:`web-${Date.now()}`,text:evidence,source:'DuckDuckGo Search'}]);largeJavaScriptLM.learn(evidence,{passes:1,rate:.003});toolResults.push({tool:'DuckDuckGo',count:sources.length,summary:`Retrieved ${sources.length} current web sources.`});confidence=Math.max(confidence,.7);}else toolResults.push({tool:'DuckDuckGo',count:0,summary:'No current web results were returned.'});}catch(e){toolResults.push({tool:'DuckDuckGo',error:String(e.message||e)});}}
