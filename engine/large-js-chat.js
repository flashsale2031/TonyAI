...
export async function largeJSChat({messages=[],attachments=[]}={},assistant=null){
 ...
 if(!wantsImage){
  ...
  try{const deterministic=replicate({message:latest},{allowModel:false});if(!reply&&!deterministic.requiresModel){reply=deterministic.reply;confidence=deterministic.confidence;toolResults.push({tool:'replicated-engine',operation:deterministic.operation,summary:deterministic.reply});}}catch(e){toolResults.push({tool:'replicated-engine',error:String(e.message||e)});}
