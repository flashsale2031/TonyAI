...
const normalizeUrl = value => {
  try { const url = new URL(String(value || '').trim()); url.hash = ''; url.hostname = url.hostname.toLowerCase(); return url.toString().replace(/\/$/, ''); }
...
function freshnessScore(page = {}) { const dateText = clean(page.published || page.date || page.modified || page.description); const match = dateText.match(/\b(20\d{2})[-/](\d{1,2})[-/](\d{1,2})\b/); if (!match) return 0; const ageDays = Math.max(0, (Date.now() - Date.parse(`${match[1]}-${match[2]}-${match[3]}`)) / 86400000); return Math.max(0, 10 - Math.min(10, ageDays / 365)); }
...
function documentTextFor(raw, facts) { return [raw.title, raw.description, raw.snippet, ...facts].filter(Boolean).join(' '); }
function escapeMarkdown(value) { return clean(value).replace(/\\/g, '\\\\').replace(/([`*_{}\[\]()#+.!|<>])/g, '\\$1'); }
function escapeCodeFence(value) { return String(value ?? '').replace(/\u0000/g, '').replace(/```/g, '` ` `').replace(/\r\n?/g, '\n').trim(); }
function safeLanguage(value) { const language = clean(value).toLowerCase().replace(/[^a-z0-9+#.-]/g, ''); return language || 'text'; }
function detectCode(raw, facts) { const candidate = String(raw.code || raw.codeSnippet || '').trim(); if (candidate) return { language: safeLanguage(raw.language), code: escapeCodeFence(candidate) }; const text = [raw.text, raw.content, ...facts].filter(Boolean).join('\n'); const fence = text.match(/```([a-z0-9+#.-]*)\s*\n([\s\S]*?)```/i); return fence ? { language: safeLanguage(fence[1]), code: escapeCodeFence(fence[2]) } : null; }
...
export function buildResultRecord(raw = {}, profile = buildPageSearchProfile('')) {
  ...
  return { id: url || `result-${Math.random().toString(36).slice(2)}`, url, domain: domainOf(url), title: clean(raw.title || raw.source?.title || url), description: clean(raw.description || raw.source?.description || raw.snippet), facts, code, extraction: extraction || { facts, characteristics: [] }, characteristics, matchedCharacteristics: characteristics.filter(c => pageText.toLowerCase().includes(clean(c).toLowerCase())), scores: { total: score, characteristicHits, lexical, authority, freshness, evidence, extraction: extractionScore }, collectedAt: new Date().toISOString() };
}
...
function renderRecord(record, index) {
  ...
  if (record.code?.code) lines.push('', `Code (${safeLanguage(record.code.language)}):`, '```' + safeLanguage(record.code.language), record.code.code, '```');
...
export class ResultsCollection {
  ...
  rebuild() { const values = [...this.results.values()]; for (const record of values) { record.relevance = Number((record.scores.total + record.matchedCharacteristics.length * 7).toFixed(3)); record.profileMatch = Number(((record.matchedCharacteristics.length / Math.max(1, record.characteristics.length)) * 100).toFixed(1)); } values.sort((a, b) => b.relevance - a.relevance || b.scores.authority - a.scores.authority || a.url.localeCompare(b.url)); this.results = new Map(values.map(record => [record.url, record])); this.revision += 1; return this.ordered(); }
  ...
  organize() { const groups = new Map(); for (const record of this.ordered()) { const key = record.domain || 'unknown'; if (!groups.has(key)) groups.set(key, []); groups.get(key).push(record); } return [...groups.entries()].map(([domain, results]) => ({ domain, results })); }
