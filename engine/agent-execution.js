...
function auth(value) { return TOKEN.length > 0 && value === `Bearer ${TOKEN}`; }
...
function trimOutput(value) { const text = String(value || ''); return text.length > MAX_OUTPUT ? `${text.slice(0, MAX_OUTPUT)}\n[output truncated]` : text; }
...
async function read(relative) {
  const filename = safePath(relative); const stat = await fs.stat(filename);
...
async function write(relative, content) {
  const filename = safePath(relative); const text = String(content ?? '');
  if (!text || text.length > MAX_WRITE) throw new Error('Write must contain 1 to 500,000 characters');
  await fs.mkdir(path.dirname(filename), { recursive: true });
  await fs.writeFile(filename, text, 'utf8');
  return { path: relative, bytes: Buffer.byteLength(text), ok: true };
}
function run(command, args = []) {
  const allowed = new Map([
    ...
    ['lint', ['npm', ['run', 'lint']]],
    ['test-chatbox', ['npm', ['run', 'test:chatbox-100']]]
  ]);
  const spec = allowed.get(command); if (!spec) throw new Error('Command is not allowlisted');
...
async function githubWrite(request) {
  const pathName = String(request?.path || '').replace(/^\/+/, '');
  if (!pathName || pathName.includes('..')) throw new Error('Invalid GitHub path');
  const content = String(request?.content ?? '');
  if (!content || content.length > MAX_WRITE) throw new Error('GitHub write must contain 1 to 500,000 characters');
...
export async function executeAgent(request, authorization) {
  ...
  try {
    if (action === 'diagnostics') return { status: 200, body: { ok: true, workspace: ROOT, node: process.version, githubConfigured: Boolean(process.env.GITHUB_TOKEN), actions: ['diagnostics', 'list', 'read', 'write', 'run', 'github-read', 'github-write'] } };
    ...
    if (action === 'write') return { status: 200, body: await write(request.path, request.content) };
    ...
    if (action === 'github-write') return { status: 200, body: await githubWrite(request) };
