...
export class UltimateAssistant{
 ...
 async chat({messages,attachments=[]}){
  ...
  let plan=null,verification=null,researchResults=[],inspection=null,queued=null;
  ...
  if(process.env.OPENAI_API_KEY&&this.cfg.researchEnabled&&/\b(research|find|look up|latest|compare|sources?|what is|who is|how much)\b/i.test(latest)&&!taskLike){try{await this.ensureBrowser();const p=await this.context.newPage();researchResults=await research(p,[latest],this.cfg.researchMaxSources);await p.close();}catch(e){researchResults=[{error:String(e.message||e)}];}}
 ...
 capabilities(){return {chat:true,offlineFallback:true,research:this.cfg.researchEnabled,browserInspection:true,browserAutomation:true,visualFallback:true,planning:true,verification:true,policy:true,surveyClassifier:true,profileBoundary:true,consensus:true,deterministicValidation:true,outcomeMemory:true,queue:true,audit:true,recovery:true,attachments:true,multimodalImages:true,fileGeneration:true,zipGeneration:true,securityBoundaries:true,autoSubmit:this.cfg.autoSubmit};}
