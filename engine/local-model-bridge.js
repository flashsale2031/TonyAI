...
// Deterministic tools -> Pure JavaScript 40B LargeLM -> local utilities -> server.
...
(() => {
  ...
  let largePromise;
  const ensureLarge=async()=>{if(window.__TONYPureLargeLM)return window.__TONYPureLargeLM;if(!largePromise)largePromise=import('/engine/large-language-model-pure-40b.js').then(m=>{const r=m.createPureJavaScriptLargeLanguageModel40B({experts:8,contextBudget:140000000,memoryTurns:1024,maxTokens:3200,seed:'tony-large-pure-v40b'});window.__TONYPureLargeLM=r;return r}).catch(e=>{window.__TONYLocalLargeModelError=String(e?.message||e);return null});return largePromise;};
  const largeReply=async(prompt)=>{const m=await ensureLarge();if(!m)return null;try{const r=await m.chat(prompt,{maxTokens:general.test(prompt)?3200:2000,temperature:general.test(prompt)?.52:.46});return r?.reply&&r.reply.trim().length>2?r:null}catch{return null}};
  window.fetch=async(url,options={})=>{if(!isChat(url,options))return original(url,options);let body={};try{body=JSON.parse(options.body||'{}')}catch{return original(url,options)}const messages=Array.isArray(body.messages)?body.messages:[],latest=[...messages].reverse().find(m=>m?.role==='user')?.content||'';try{
    ...
    if(latest){const r=await largeReply(latest);if(r)return response({...r,requiresHuman:false,openaiRequired:false,pretrainedRequired:false,neuralBackbone:'none',pureJavaScript:true,largeLocalModel:true,largeModelParameters:40000000000,comparisonTarget:'8b-neural-network-benchmark-target',attachmentCount:(body.attachments||[]).length});}
