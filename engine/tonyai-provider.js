...
function localImage(prompt, options={}) {
 ...
 const seed = hash(prompt); const bg = palette(seed); const accent = palette(seed >>> 4); const safePrompt = escapeXml(prompt).slice(0, 220);
...
export const tonyAIProvider = {
 name:'tonyai-native', version:'1.0.0', external:false, openaiCompatible:false,
 chat({messages=[],max_tokens=256,temperature=.38}={}) { const prompt=messagesToPrompt(messages); const latest=clean([...messages].reverse().find(m=>m?.role==='user')?.content); const reply=largeJavaScriptLM.synthesize(latest,{retrieved:largeJavaScriptLM.retrieve(latest,8),toolResults:[],sources:[]}) || largeJavaScriptLM.generate(latest,{maxTokens:Math.min(Number(max_tokens)||256,2048),temperature:Number(temperature)||.38}); return {id:`tony-${Date.now().toString(36)}-${hash(prompt).toString(16)}`,object:'chat.completion',created:Math.floor(Date.now()/1000),model:'tonyai-native',choices:[{index:0,message:{role:'assistant',content:reply},finish_reason:'stop'}],usage:{prompt_tokens:prompt.split(/\s+/).filter(Boolean).length,completion_tokens:reply.split(/\s+/).filter(Boolean).length,total_tokens:(prompt+' '+reply).split(/\s+/).filter(Boolean).length}}; },
