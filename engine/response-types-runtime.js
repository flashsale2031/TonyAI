...
// Include curated types before generated types so specific terms such as
// "president", "recipe", and "programming" beat broad context terms.
...
const BROAD_TERMS = new Set(['current', 'now', 'today', 'latest', 'official', 'published', 'verified']);
...
const CONTEXT_SIGNALS = [
  ['current', ['current', 'now', 'today', 'latest']], ['official', ['official', 'authorized', 'published']],
  ['verified', ['verified', 'confirmed', 'validated']], ['local', ['local', 'nearby', 'regional']],
  ['historical', ['historical', 'history', 'background']], ['quantitative', ['number', 'count', 'price', 'rate', 'percentage', 'average', 'median']],
  ['comparison', ['compare', 'difference', 'versus', 'vs']], ['ranking', ['best', 'top', 'ranking']],
  ['schedule', ['schedule', 'date', 'time', 'when']], ['location', ['where', 'location', 'near']],
  ['process', ['how', 'steps', 'process']], ['evidence', ['evidence', 'source', 'citation', 'proof']]
];
...
function classifyAnchor(question) {
  ...
  for (const entry of ANCHOR_INDEX) {
    ...
    for (const keyword of entry.keywords) {
      if (has(question, keyword)) score += BROAD_TERMS.has(keyword.toLowerCase()) ? 1 : keyword.length > 5 ? 3 : 2;
    }
...
export function classifyResponseType(question = '') {
  ...
  return variant ? { ...variant, score: anchor.score + context.score, anchorType: anchor.type.id }
